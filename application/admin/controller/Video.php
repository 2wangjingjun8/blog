...
class Video extends Publics
{
    public function index()
    {
		...
		->alias('vd')
		->join('blog_user bu','vd.user_id = bu.id')

		->field('vd.id,vd.name,vd.url,vd.create_time,bu.real_name')->where(array('vd.user_id'=>$this->user_id))->order('id desc')->paginate($this->prePage=10);
                            ...
                            foreach($v AS $key=>$value){
							   if($key == 'create_time'){
								   ...
								   continue;
									...
									$html.='<td><video width="180" height="120" controls="controls">';
									$html.=' <source src="'.$value.'" type="video/ogg">';
